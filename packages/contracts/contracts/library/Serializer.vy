...
INCLUSION_PROOF_START: constant(int128) = 101
...
def decodeInclusionProof(
  ...
  size: int128 = 111 + _numNodes * 41
  leftOrRight: uint256 = convert(slice(_proofs, start=INCLUSION_WITNESS_START + 111 +  size * _index + _i * 41, len=1), uint256)
  amount: uint256 = convert(slice(_proofs, start=INCLUSION_WITNESS_START + 111 +  size * _index + _i * 41 + 1, len=8), uint256)
  proofElement: bytes32 = extract32(_proofs, INCLUSION_WITNESS_START + 111 +  size * _index + _i * 41 + 9, type=bytes32)
  return (
    leftOrRight,
    amount,
    proofElement
...
def decodeInclusionWitness(
  ...
  sig: bytes[65] = slice(_proofs, start=INCLUSION_WITNESS_START + 36 + size * _index, len=65)
  return (
    txBytesOffset,
    txBytesSize,
    segment,
