...
describe('SparceMerkleTree', function() {
  ...
  describe('verify', function() {
    ...
    it('normal', function() {
      const leaf = Array.from(Array(Math.pow(2, 16)), () => null);
      ...
      const tree = new SparseMerkleTree(16, leaf);
      ...
      assert.equal(SparseMerkleTree.verify(leaf[2], 2, root, proof), true);
    ...
    it('odd leaves', function() {
      ...
      assert.equal(proof.length, 512);
      assert.equal(SparseMerkleTree.verify(leaf[1], 1, root, proof), true);
    });
