...
describe('SparceMerkleTree', function() {
  ...
  describe('verify', function() {
    ...
    it('normal', function() {
      ...
      assert.equal(tree.verify(leaves[2], 2, root, proof), true);
      assert.equal(tree.verify(unknownLeaf, 2, root, proof), false);
      assert.equal(tree.verify(zeroHash, 7, root, tree.proof(7)), true);
    });
    ...
    it('odd leaves', function() {
      ...
      assert.equal(proof.length, 512);
      assert.equal(tree.verify(leaves[1], 1, root, proof), true);
    });
